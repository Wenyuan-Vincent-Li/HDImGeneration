...
class WDiscriminator(nn.Module):
    def __init__(self, opt):
        ...
        N = int(opt.nfc)  # 32
        self.head = ConvBlock(opt.input_nc + opt.label_nc, N, opt.ker_size, opt.padd_size,
...
class GeneratorConcatSkip2CleanAdd(nn.Module):
    def __init__(self, opt):
        ...
        self.tail = nn.Sequential(
            nn.Conv2d(max(N, opt.min_nfc), opt.input_nc, kernel_size=opt.ker_size, stride=1, padding=opt.padd_size),
