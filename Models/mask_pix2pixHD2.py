...
class GeneratorConcatSkip2CleanAdd(nn.Module):
    def __init__(self, input_nc, output_nc, opt, ngf=64, n_downsampling=3, n_blocks=9, norm_layer=nn.BatchNorm2d,
        ...
        model += [nn.ReflectionPad2d(3), nn.Conv2d(ngf, output_nc, kernel_size=7, padding=0), nn.Softmax2d()]
        self.model = nn.Sequential(*model)

    def forward(self, x, y):
        '''
        :param x: noise layer: z vectors [None, 4, x, y]
        :param y: prev mask: previous generated mask [None, 4, x, y]
        :return:
        '''
        x = self.model(x)
...
class WDiscriminator(nn.Module):
    def __init__(self, input_nc, opt, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d,
        ...
        for i in range(num_D):
            netD = NLayerDiscriminator(input_nc, opt, ndf, n_layers, norm_layer, use_sigmoid,
                                       getIntermFeat)  ## (8, 64, 3, instance, False, True)
            if getIntermFeat:
    ...
    def forward(self, input):
        if input.shape[1] == 1:
             input = mask2onehot(input, self.opt.label_nc)
        num_D = self.num_D  ## 2
        result = []
...
def init_models(opt):
    ...
    netG = GeneratorConcatSkip2CleanAdd(opt.label_nc, opt.label_nc, opt).to(opt.device)
    ...
    netD = WDiscriminator(opt.label_nc, opt, norm_layer=norm_layer, getIntermFeat=not opt.no_ganFeat_loss,
