...
def train_single_scale(dataloader, netD, netG, reals, Gs, Zs, in_s, NoiseAmp, opt):
    ...
    for epoch in range(opt.niter):  # niter = 2000
        ...
        for j, data in enumerate(dataloader):
            ...
            # train with real
            netD.zero_grad()
            output = netD(data['image'], data['label']).to(
            ...
            if left < right:
                ...
                for idx, value in enumerate(Zs):
                    ...
                    fixed_data_label.append(torch.cat(
                        (fixed_data_label_temp[idx][left:, :, :, :], fixed_data_label_temp[idx][:right, :, :, :]),
                        dim=0))
            ...
            if (j == 0) & (epoch == 0):  # first iteration training in this level
                if Gs == []:
                    ...
                    prev = draw_concat(Gs, Zs_prime, data['down_scale_label'], reals, NoiseAmp, in_s, 'rand', m_noise,
                                       m_image, opt)
            ...
            gradient_penalty = functions.calc_gradient_penalty(netD, data['image'], fake, data['label'],
                                                               opt.lambda_grad)
            ...
            if left < right:
                ...
                fixed_mask_temp = torch.cat((fixed_data['label'][left:, :, :, :], fixed_data['label'][:right, :, :, :]),
                                            dim=0)
                # fixed_mask_temp = m_image(fixed_mask_temp)
        ...
        if epoch % 50 == 0 or epoch == (opt.niter - 1):
            ...
            plt.imsave('%s/G(z_opt)_%d.png' % (opt.outf, epoch),
                       functions.convert_image_np(netG(Z_opt.detach(), z_prev, fixed_mask_temp).detach()), vmin=0,
                       vmax=1)
            ...
            for G, Z_opt, mask, real_curr, real_next, noise_amp in zip(Gs, Zs, masks, reals, reals[1:], NoiseAmp):
                if count == 0:
                    ...
                    z = functions.generate_noise(
                        [opt.nc_z, Z_opt.shape[2] - 2 * pad_noise, Z_opt.shape[3] - 2 * pad_noise], opt.batchSize)
                ## z [None, 3, 32, 32]
