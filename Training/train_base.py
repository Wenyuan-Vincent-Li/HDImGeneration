        ...
        for j, data in enumerate(dataloader):
            ...
            if (j==0) & (epoch == 0): # first iteration training in this level
                if Gs == []:
                    ...
                    prev = draw_concat(Gs, data['down_scale_label'], reals, NoiseAmp, in_s, 'rand', m_noise, m_image, opt)
...
def draw_concat(Gs, masks,reals,NoiseAmp,in_s,mode,m_noise,m_image,opt):
    '''
    :param Gs: [G0]
        ...
        if mode == 'rand' or mode == 'rec':
            ...
            for G, mask, real_curr,real_next,noise_amp in zip(Gs, masks,reals,reals[1:],NoiseAmp):
                if count == 0:
                    ...
                    z = functions.generate_noise([opt.nc_z, real_curr[0], real_curr[1]], opt.batchSize)
                ## z [None, 3, 32, 32]
