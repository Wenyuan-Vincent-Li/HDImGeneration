...
#                     z_prev = torch.full([opt.batchSize,opt.nc_z,opt.nzx,opt.nzy], 0, device=opt.device)
            ...
            if alpha!=0: ## alpha = 10 calculate the reconstruction loss
                ...
                print(Z_opt.shape, z_prev.shape, fixed_mask_temp.shape, fixed_image_temp.shape)
                exit()
        ...
        if epoch % 50 == 0 or epoch == (opt.niter-1):
            ...
            plt.imsave('%s/original_image_%d.png' % (opt.outf, epoch), functions.convert_image_np(fixed_data['image']), vmin=0,
                       vmax=1)
