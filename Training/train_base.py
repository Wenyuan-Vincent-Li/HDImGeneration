...
from InputPipeline.base_dataset import __scale_width
    ...
    for epoch in range(opt.niter): # niter = 2000
        ...
        for j, data in enumerate(dataloader):
            ...
            if left < right:
                ...
                Zs_prime = []
                for i in Zs:
                    Zs_prime.append(torch.cat((i[left:,:,:,:], i[:right,:,:,:]), dim = 0))
            ...
            if (j==0) & (epoch == 0): # first iteration training in this level
                if Gs == []:
                    ...
                    prev = draw_concat(Gs, Zs_prime, data['label'], maskTransform, reals, NoiseAmp, in_s, 'rand', m_noise, m_image, opt)
                    ...
                    z_prev = draw_concat(Gs, Zs_prime, data['label'], maskTransform, reals, NoiseAmp, in_s, 'rec', m_noise, m_image, opt) ## [1, 3, 33, 33]
...
def draw_concat(Gs,Zs,mask,maskTransforms, reals,NoiseAmp,in_s,mode,m_noise,m_image,opt):
    '''
    :param Gs: [G0]
    ...
    if len(Gs) > 0:
        if mode == 'rand':
            ...
            for G,Z_opt, maskTransform, real_curr,real_next,noise_amp in zip(Gs,Zs,reversed(maskTransforms),reals,reals[1:],NoiseAmp):
                ...
                G_z = G(z_in.detach(),G_z, mask_curr) ## [1, 3, 26, 26] output of previous generator
        ...
        if mode == 'rec':
            ...
            for G,Z_opt,maskTransform,real_curr,real_next,noise_amp in zip(Gs,Zs,reversed(maskTransforms), reals,reals[1:],NoiseAmp):
                ...
                mask_curr = maskTransform(mask)
                mask_curr = m_image(mask_curr)
                G_z = G(z_in.detach(),G_z, mask_curr) ## [1, 3, 26, 26]
